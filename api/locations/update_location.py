...
import json
from get_coords import query_county_locations, query_town_locations, query_region_locations, query_utility_locations, query_state_senate_locations, query_assembly_locations
    ...
    if table_name in ["Utility Service Territories", "Utility Service Territories Test"]:
        for i, item in enumerate(data):
          ...
          if "Municipal Utility:" in name:
            ...
            name = "Municipal Utility: " + municipal_subname
          updates.append({
              "id": id,
              column_name: name,
              "coordinates": coordinates_json,
...
def update_location_data():
    ...
    county_data_updated = update_table_data(county_data, "Counties", "county")
    town_data_updated = update_table_data(town_data, "Towns", "town")
    region_data_updated = update_table_data(region_data, "Regions", "region")
    utility_data_updated = update_table_data(utility_data, "Utility Service Territories", "utility_service_territories")
    state_senate_data_updated = update_table_data(state_senate_data, "State Senate Districts", "state_senate_district")
    assembly_data_updated = update_table_data(assembly_data, "Assembly Districts", "assembly_district")
