...
def nyiso_to_database() -> None:
    ...
    def nyiso_to_database_helper(projects, sheet_name):
        for project in projects:
            ...
            # turns last_updated Timestamp objects into datetime strings
            if project.get("last_updated", None) is not None:
                try:
                    ymd = project.get("last_updated").strftime("%Y-%m-%d")
                except Exception as exception:
                    print(exception)
                project["last_updated"] = ymd
